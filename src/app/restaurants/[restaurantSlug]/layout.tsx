...
export default async function RestaurantLayout({ children, params }: RestaurantLayoutProps) {
  ...
  return (
    <ToastProvider>
      <div className="restaurant-public-shell bg-canvas flex min-h-dvh flex-col" style={brandStyle}>
        {adminPreview ? (
          <div className="bg-warning/15 text-warning px-4 py-2 text-center text-xs font-semibold">
        ...
        {isPublished ? (
          <PageViewTracker restaurantId={restaurant.id} slug={restaurant.slug} />
        ) : null}
      </div>
    </ToastProvider>
  );
