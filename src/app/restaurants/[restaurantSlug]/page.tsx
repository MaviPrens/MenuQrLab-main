  ...
  const renderSection = (key: string) => {
    switch (key) {
        ...
        return popular.length > 0 ? (
          <section key={key} className="mt-10 px-5">
            <div className="mb-4 flex items-end justify-between">
              <h2 className="font-heading text-h3 text-text-primary font-bold">Popular choices</h2>
              <Link
                href={routes.restaurant.menu(restaurant.slug)}
                className="text-small text-primary focus-visible:outline-primary font-bold hover:underline focus-visible:outline-2 focus-visible:outline-offset-2"
              >
                View full menu
              </Link>
            </div>
      ...
      case "opening-hours":
        return (
          <section key={key} className="mt-10 px-5">
            <h2 className="font-heading text-h3 text-text-primary mb-4 font-bold">Opening hours</h2>
      ...
      case "location":
        return (
          <section key={key} className="mt-10 px-5">
            <h2 className="font-heading text-h3 text-text-primary mb-4 font-bold">
            ...
            <Link
              ...
              className="text-button text-primary focus-visible:outline-primary mt-4 inline-flex items-center gap-1.5 font-bold hover:underline focus-visible:outline-2 focus-visible:outline-offset-2"
            >
    ...
    <div className="max-w-shell mx-auto w-full">
      ...
      {description ? (
        <section className="mt-10 mb-12 px-5">
          <div className="border-outline-variant/40 bg-surface-warm rounded-[20px] border p-6">
            <h2 className="font-heading text-h3 text-primary font-bold">
              Made fresh. Served fast.
            </h2>
            <p className="text-small text-text-secondary mt-2 leading-relaxed">{description}</p>
          </div>
        </section>
      ) : null}
    </div>
