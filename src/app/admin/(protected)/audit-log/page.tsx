...
export default function AuditLogPage() {
  ...
  const accessChanges = records.filter(
    (r) =>
      r.resourceType === "admin-user" || r.resourceType === "team" || r.action.includes("access"),
  ).length;
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-text-primary">
            {ROLE_LABELS[row.original.actorRole] ?? titleCase(row.original.actorRole)}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span
              className={
                intent === "danger"
                  ? "border-danger/20 bg-danger/10 text-danger inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-semibold"
                  : intent === "success"
                    ? "border-success/20 bg-success/10 text-success inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-semibold"
                    : intent === "warning"
                      ? "border-warning/20 bg-warning/10 text-warning inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-semibold"
                      : "border-border bg-surface-container text-text-secondary inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-semibold"
              }
            >
              {row.original.action}
            </span>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-text-secondary">
            ...
            <span className="text-text-tertiary ml-1">· {row.original.resourceId}</span>
          </span>
        ),
      },
      {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="border-info/30 bg-info/5 flex items-start gap-3 rounded-[16px] border p-4">
        <Icon name="ShieldCheck" className="text-info mt-0.5 size-5 shrink-0" aria-hidden />
      ...
      >
        <div className="border-border bg-surface mb-0.5 flex items-center gap-1 rounded-[12px] border p-1">
          <button
            type="button"
            ...
            className={
              view === "table"
                ? "bg-canvas text-small text-text-primary shadow-card flex h-9 items-center gap-1.5 rounded-[8px] px-3 font-semibold"
                : "text-small text-text-secondary hover:text-text-primary flex h-9 items-center gap-1.5 rounded-[8px] px-3 font-medium"
            }
          >
