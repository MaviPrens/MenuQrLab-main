...
import { restaurantCreateSchema, isValidSlug, type RestaurantCreateInput } from "@/domain/schemas";
...
export default function NewRestaurantPage() {
  ...
  }, [watchedName, watchedDisplay, watchedSlug, slugStatus, restaurantTypes, serviceModels]);
  ...
  return (
    <form onSubmit={onCreate} className="flex flex-col gap-6">
      <AdminPageHeader
        ...
        description="Create a draft restaurant. This never publishes, generates QR codes, campaigns or accounts."
        breadcrumb={[
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[200px_1fr_240px]">
            ...
            {SECTIONS.map((s) => (
              <li key={s.id}>
                <a
                  ...
                  className={cn(
                    "text-small flex min-h-9 items-center rounded-[10px] px-3 font-medium transition-colors",
                    active === s.id
        ...
        <div className="flex min-w-0 flex-col gap-6">
          ...
          <AdminSection id="section-review" title="Review" icon="CheckCircle2">
            ...
            <p className="bg-surface text-text-secondary mt-4 rounded-[12px] p-3 text-xs">
              Creating this restaurant produces a <strong>draft</strong> only. It does not publish,
              generate QR codes, create campaigns or accounts.
            </p>
          </AdminSection>
        </div>
        ...
        <aside className="hidden lg:block">
          <div className="border-border bg-canvas shadow-card sticky top-20 flex flex-col gap-3 rounded-[16px] border p-4">
            <p className="text-small text-text-primary font-semibold">Completion</p>
            ...
            <p className="text-text-secondary text-xs">{completion}% of key fields complete</p>
      ...
      <StickyActionBar
        info={
          <span className="flex items-center gap-1.5">
            <Icon name="Info" className="text-text-tertiary size-4" aria-hidden />
...
function ReviewRow({ label, value, status }: { label: string; value?: string; status?: string }) {
  return (
    <li className="border-border flex items-center justify-between gap-3 border-b pb-2 last:border-0">
      <span className="text-text-secondary">{label}</span>
      <span className="text-text-primary flex items-center gap-1.5 text-right font-medium">
        {value || "—"}
        {status === "available" ? (
          <Icon name="CheckCircle2" className="text-success size-4" aria-hidden />
        ) : status === "taken" ? (
          <Icon name="XCircle" className="text-danger size-4" aria-hidden />
        ) : null}
      </span>
    </li>
  );
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <dd className="text-text-primary truncate font-medium">{value}</dd>
    </div>
  );
}
