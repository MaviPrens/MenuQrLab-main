      ...
      >
        {assets.length === 0 ? (
          ...
          <EmptyState
            ...
            description="No assets match the selected type filter."
            icon="Search"
          />
        ) : (
          <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.map((asset) => (
              <li
                key={asset.id}
                className="border-border bg-canvas shadow-card flex flex-col overflow-hidden rounded-[16px] border"
              >
                <div className="bg-surface relative h-40 w-full">
                  <Image
                  ...
                  />
                  <span className="bg-navy/80 absolute top-2 left-2 rounded-full px-2 py-0.5 text-[11px] font-semibold text-white">
                    {titleCase(asset.type)}
                  </span>
                  {!asset.altText ? (
                    <span className="bg-warning/90 absolute top-2 right-2 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold text-white">
                ...
                <div className="flex flex-1 flex-col gap-2 p-3">
                  <p className="text-text-primary truncate font-medium" title={asset.filename}>
                    {asset.filename}
                  </p>
                  ...
                  <div className="mt-auto flex flex-wrap items-center gap-2">
                    ...
                    <span className="bg-surface-muted text-text-secondary inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold">
    ...
    if (mode === "create") {
      ...
      toast({
        ...
        description: uploadedUrl
          ...
          : "Stored as a placeholder (configure Blob to save files).",
        intent: "success",
      });
    } else if (asset) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      <div
        ...
        className="border-border bg-canvas shadow-lift relative w-full max-w-lg overflow-hidden rounded-[20px] border"
      >
        ...
        <form onSubmit={onSubmit} className="flex max-h-[70vh] flex-col gap-4 overflow-y-auto p-5">
          <div className="flex items-start gap-4">
            <div className="border-border bg-surface relative size-24 shrink-0 overflow-hidden rounded-[12px] border">
              <Image
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                accept="image/*"
                onChange={handleFile}
                className="text-small text-text-secondary file:border-input-border file:bg-canvas file:text-small file:text-text-primary hover:file:bg-surface block w-full file:mr-3 file:min-h-11 file:cursor-pointer file:rounded-[12px] file:border file:px-4 file:font-semibold"
              />
              <p className="text-text-tertiary text-xs">
                Uploads to storage when Blob is configured; otherwise a placeholder is stored on
                save.
              </p>
            </div>
          </div>
