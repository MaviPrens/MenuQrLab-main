...
function normalize(existing: OpeningHours[]): OpeningHours[] {
  ...
  return DAYS_OF_WEEK.map((day) => {
    ...
    return found ?? { day, status: "closed" as const, periods: [], specialHours: null };
  });
}
...
export default function OpeningHoursPage() {
      ...
      days.map((d, i) =>
        i === index
          ? {
              ...
              status,
              periods:
                status === "open" && d.periods.length === 0
    ...
    mutate(
      days.map((d, i) =>
        i === dayIndex
          ? {
              ...d,
              periods: d.periods.map((p, pi) => (pi === periodIndex ? { ...p, ...patch } : p)),
            }
          : d,
      ),
    );
  ...
  const isOpenNow =
    today?.status === "open" &&
    today.periods.some(
      (p) => TIME_RE.test(p.open) && TIME_RE.test(p.close) && p.open <= nowHm && nowHm < p.close,
    );
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_300px]">
        <div className="flex min-w-0 flex-col gap-6">
          ...
          >
            ...
            {!timezone ? (
              <div className="border-warning/30 bg-warning/5 text-small text-warning mt-3 flex items-start gap-2 rounded-[12px] border p-3">
                <Icon name="AlertTriangle" className="mt-0.5 size-4 shrink-0" aria-hidden />
                <span>
                  Time-zone review required — a missing time zone makes open/closed status
                  unreliable.
                </span>
              </div>
            ) : null}
          ...
          >
            <ul className="flex flex-col gap-3">
              {days.map((day, dayIndex) => {
                ...
                return (
                  <li key={day.day} className="border-border bg-surface rounded-[12px] border p-4">
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="flex min-w-[150px] items-center gap-3">
                        <button
                          ...
                          onClick={() =>
                            setStatus(dayIndex, day.status === "open" ? "closed" : "open")
                          }
                          className="border-input-border bg-canvas data-[on=true]:bg-primary relative inline-flex h-7 w-12 shrink-0 items-center rounded-full border transition-colors"
                          data-on={day.status === "open"}
                        >
                          <span
                            className="bg-text-tertiary inline-block size-5 translate-x-1 rounded-full transition-transform data-[on=true]:translate-x-6 data-[on=true]:bg-white"
                            data-on={day.status === "open"}
                            aria-hidden
                          />
                        </button>
                      ...
                      {day.status === "closed" ? (
                        <span className="bg-surface-muted text-text-secondary inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold">
                          ...
                          {day.periods.map((period, periodIndex) => (
                            <div key={periodIndex} className="flex flex-wrap items-center gap-2">
                              <Input
                                ...
                                value={period.open}
                                onChange={(e) =>
                                  setPeriod(dayIndex, periodIndex, { open: e.target.value })
                                }
                              ...
                              <Input
                                ...
                                value={period.close}
                                onChange={(e) =>
                                  setPeriod(dayIndex, periodIndex, { close: e.target.value })
                                }
                                className="h-11 w-[120px]"
                              />
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-20 flex flex-col gap-4">
            <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
              <p className="text-small text-text-primary font-semibold">Live public status</p>
              <div className="mt-2 flex items-center gap-2">
                <Icon
                  name={isOpenNow ? "CheckCircle2" : "Moon"}
                  className={isOpenNow ? "text-success size-4" : "text-text-secondary size-4"}
                  aria-hidden
                />
                <span
                  className={
                    isOpenNow
                      ? "text-small text-success font-semibold"
                      : "text-small text-text-secondary font-semibold"
                  }
                >
            ...
            <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
              <p className="text-small text-text-primary font-semibold">Validation</p>
              {validationErrors.length === 0 ? (
                <p className="text-success mt-2 flex items-center gap-1.5 text-xs">
                  <Icon name="CheckCircle2" className="size-3.5" aria-hidden /> No schedule
                  conflicts.
                </p>
              ) : (
