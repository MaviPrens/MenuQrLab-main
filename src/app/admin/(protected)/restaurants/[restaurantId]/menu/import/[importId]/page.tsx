...
import { IMPORT_PIPELINE_STAGES, STAGE_META, type ImportPipelineStage } from "@/domain/menu-import";
...
export default function MenuImportDetailPage() {
  ...
  const isReview =
    (REVIEW_STATUSES as readonly string[]).includes(status) && record.result !== null;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="border-border bg-canvas shadow-card flex flex-wrap items-center gap-2 rounded-[12px] border p-3">
        <Icon name="FileText" className="text-text-secondary size-4" aria-hidden />
        <span className="text-small text-text-primary font-medium">{record.originalFileName}</span>
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-2">
            <div className="text-small flex items-center justify-between">
              <span className="text-text-primary font-semibold">{record.progress}% complete</span>
              ...
              return (
                <li
                  ...
                  className={`flex items-center gap-3 rounded-[10px] border px-3 py-2 ${
                    isCurrent ? "border-primary bg-surface-warm" : "border-border bg-surface"
                  }`}
                >
                  <span
                  ...
                  <span
                    className={`text-small ${isCurrent ? "text-text-primary font-semibold" : "text-text-secondary"}`}
                  >
                    {meta.label}
                  </span>
                  {isCurrent ? (
                    <span className="text-primary ml-auto text-xs font-medium">In progress…</span>
                  ) : null}
                </li>
              );
            })}
      ...
      <AdminSection title="Recent events" description="The latest pipeline events." icon="ListTree">
        <ul className="flex flex-col gap-2">
          {latestEvents.map((evt) => (
            <li
              key={evt.id}
              className="border-border bg-surface text-small flex items-center justify-between gap-3 rounded-[10px] border px-3 py-2"
            >
...
function CompletedView({ record, restaurantId }: { record: MenuImport; restaurantId: string }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="border-success/30 bg-success/5 flex flex-col items-center gap-3 rounded-[16px] border px-6 py-10 text-center">
      ...
      {record.result ? (
        <AdminSection
          ...
          description="The committed result, read-only."
          icon="Braces"
        >
          <pre className="border-border bg-navy-deep text-surface max-h-96 overflow-auto rounded-[12px] border p-4 text-xs leading-relaxed">
