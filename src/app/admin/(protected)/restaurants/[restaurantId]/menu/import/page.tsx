...
export default function MenuImportLandingPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="border-warning/30 bg-warning/5 text-small text-warning flex items-start gap-2 rounded-[12px] border p-3">
        ...
        <span>
          ...
          expect to correct item names. Every import requires human review before anything is saved,
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
          <AdminSection
            title="Upload a menu PDF"
            description="Drag a file here or browse."
            icon="FileUp"
          >
            <div className="flex flex-col gap-4">
              <label htmlFor={fileInputId} className="text-small text-text-primary font-semibold">
                Menu PDF file
              </label>
              <div
                role="button"
                ...
                onDrop={onDrop}
                className={`focus-visible:outline-primary flex cursor-pointer flex-col items-center justify-center gap-2 rounded-[16px] border-2 border-dashed px-6 py-10 text-center transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 ${
              ...
              >
                ...
                <p id={`${fileInputId}-hint`} className="text-text-secondary text-xs">
              ...
              {validated ? (
                <div className="border-success/30 bg-success/5 flex flex-col gap-3 rounded-[12px] border p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="text-small text-text-primary flex min-w-0 items-center gap-2 font-semibold">
                      <Icon name="FileText" className="text-success size-4 shrink-0" aria-hidden />
                      <span className="truncate">{validated.file.name}</span>
                    </p>
                    <span className="text-text-secondary text-xs">
                      {formatBytes(validated.file.size)}
                    </span>
                  </div>
                  ...
                  <p className="text-text-secondary text-xs italic">
                    Estimated — page count, type and languages are inferred locally, not from a real
                    PDF parse.
                  </p>
                </div>
              ) : null}
      ...
      >
        {visibleImports.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="text-small w-full min-w-[820px] border-collapse text-left">
              ...
              <tbody>
                {visibleImports.map((imp) => {
                  ...
                  return (
                    <tr key={imp.id} className="border-border border-b last:border-0">
                      <th
                        scope="row"
                        className="text-text-primary max-w-[220px] py-3 pr-3 font-medium"
                      >
                        <span className="block truncate">{imp.originalFileName}</span>
                      </th>
                      <td className="text-text-secondary px-3 py-3">{formatDate(imp.createdAt)}</td>
                      <td className="px-3 py-3">
                        <ProcessingStatusBadge status={imp.processingStatus} />
                      </td>
                      <td className="px-3 py-3">
...
function DetectedStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="border-border bg-canvas rounded-[10px] border px-3 py-2">
      <dt className="text-text-tertiary text-[11px] tracking-wide uppercase">{label}</dt>
      <dd className="text-small text-text-primary font-semibold">{value}</dd>
    </div>
  );
}
