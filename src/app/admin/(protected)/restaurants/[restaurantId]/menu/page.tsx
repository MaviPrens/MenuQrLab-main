...
export default function DigitalMenuManagerPage() {
  ...
  const reorderCategories = (categoryId: string, direction: -1 | 1) => {
    ...
    toast({
      ...
      description: "Draft only — nothing published.",
      intent: "success",
    });
  };
  ...
  const handleRemove = () => {
    ...
    toast({
      title: `${removeTarget.kind === "category" ? "Category" : "Product"} removed`,
      intent: "success",
    });
        ...
        <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-small text-text-primary flex items-center gap-2 font-semibold">
              <Icon name="FileUp" className="text-primary size-4" aria-hidden />
              Recent PDF imports
            </p>
              ...
              return (
                <li key={imp.id}>
                  <Link
                    ...
                    className="border-border bg-surface hover:bg-surface-muted focus-visible:outline-primary flex min-h-11 flex-wrap items-center justify-between gap-2 rounded-[12px] border px-3 py-2 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2"
                  >
            ...
            return (
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <PermissionGate user={user} permission={PERMISSIONS.MENU_EDIT}>
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        onClick={() => cycleCategoryStatus(category)}
                      >
              ...
              >
                <div className="flex flex-col gap-3">
                  {items.length === 0 ? (
                    <p className="border-border bg-surface text-small text-text-secondary rounded-[12px] border border-dashed px-4 py-6 text-center">
                    ...
                    <ul className="flex flex-col gap-2">
                      {items.map((product, productIndex) => (
                        <li
                          key={product.id}
                          className="border-border bg-canvas flex flex-col gap-3 rounded-[12px] border p-3 sm:flex-row sm:items-center sm:justify-between"
                        >
                          <div className="flex min-w-0 items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="text-text-primary flex items-center gap-2 truncate font-medium">
                                {resolveText(product.localizedName)}
                                {product.featured ? (
                                  <span className="bg-accent/20 text-warning inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold">
                  ...
                  <PermissionGate user={user} permission={PERMISSIONS.MENU_EDIT}>
                    <Button asChild variant="ghost" size="sm" className="self-start">
                      <Link
                        href={`${routes.admin.restaurantProduct(id, "new")}?categoryId=${category.id}`}
                      >
                        <Icon name="Plus" className="size-4" aria-hidden />
                        Add product
                      </Link>
...
function StatCard({ label, value, icon }: { label: string; value: number; icon: string }) {
  return (
    <div className="border-border bg-canvas shadow-card flex items-center gap-3 rounded-[16px] border p-4">
      <span className="bg-surface-warm text-primary flex size-10 shrink-0 items-center justify-center rounded-[10px]">
        <Icon name={icon} className="size-5" aria-hidden />
      </span>
      <div className="min-w-0">
        <p className="font-display text-h3 text-text-primary">{value}</p>
        <p className="text-text-secondary truncate text-xs">{label}</p>
      </div>
    </div>
  );
}
