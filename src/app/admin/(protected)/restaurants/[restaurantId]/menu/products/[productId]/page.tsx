    ...
    if (!isNew) {
      ...
      if (product) {
        reset({
          ...
          descriptionEn: product.localizedDescription
            ? resolveText(product.localizedDescription)
            : "",
          slug: product.slug,
          price: product.price,
        ...
        <div className="flex min-w-0 flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <p className="text-text-secondary text-xs">
                  Optional sizes or options. Price modifier is added to the base price
                  (informational only).
                </p>
                {fields.length === 0 ? (
                  <p className="border-border bg-surface text-small text-text-secondary rounded-[12px] border border-dashed px-4 py-4 text-center">
                    No variants added.
                  </p>
                ) : (
                  <ul className="flex flex-col gap-2">
                    {fields.map((variant, index) => (
                      <li
                        key={variant.id}
                        className="border-border flex flex-col gap-2 rounded-[12px] border p-3 sm:flex-row sm:items-end"
                      >
                        <Field
                          label="Label"
          ...
          <AdminSection title="Product image" icon="Image">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
              <div className="border-border bg-surface relative size-32 shrink-0 overflow-hidden rounded-[12px] border">
                <Image
              ...
              <div className="flex flex-col gap-2">
                ...
                <p className="border-warning/30 bg-warning/5 text-warning flex items-start gap-2 rounded-[12px] border p-3 text-xs">
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-20 flex flex-col gap-4">
            <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
              <p className="text-small text-text-primary font-semibold">Public product card</p>
              <div className="border-border mt-2 overflow-hidden rounded-[16px] border">
                <div className="bg-surface relative h-32 w-full">
                  <Image
                    src={imagePreview ?? "/placeholders/food.svg"}
                    alt=""
                    fill
                  ...
                  />
                  {featured ? (
                    <span className="bg-accent/90 text-navy absolute top-2 left-2 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold">
          ...
          <AdminSection title="Danger zone" icon="ShieldAlert">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-small text-text-secondary">
                Removing a product affects the draft menu only.
              </p>
