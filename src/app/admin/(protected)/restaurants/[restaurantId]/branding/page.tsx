        ...
        <div className="flex min-w-0 flex-col gap-6">
          ...
          <AdminSection title="Visual direction" icon="Palette">
            <Field
              ...
              htmlFor="visualDirection"
              error={errors.visualDirection?.message}
            >
          ...
          <AdminSection title="Brand colors" icon="Droplet">
                      ...
                      render={({ field }) => (
                        <>
                          <input
                            ...
                            className="border-input-border size-11 shrink-0 cursor-pointer rounded-[10px] border"
                          />
                          <Input
                            id={`color-${cf.key}`}
                            value={field.value ?? ""}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {COLOR_FIELDS.map((cf) => (
                <div key={cf.key} className="flex flex-col items-center gap-1">
                  <span
                    className="border-border size-12 rounded-[10px] border"
                    style={{ background: draftColors?.[cf.key] }}
                    aria-hidden
                  />
                  <span className="text-text-secondary text-[10px]">{cf.label}</span>
                </div>
              ))}
            </div>
            ...
            {textContrast !== null ? (
              <div
                className={
                  textContrast >= 4.5
                    ? "border-success/30 bg-success/5 text-small text-success mt-4 flex items-center gap-2 rounded-[12px] border p-3"
                    : "border-warning/30 bg-warning/5 text-small text-warning mt-4 flex items-center gap-2 rounded-[12px] border p-3"
                }
              >
                <Icon
          ...
          <AdminSection title="Logo & cover" icon="Image">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="flex flex-col gap-2">
                <span className="text-small text-text-primary font-semibold">Logo</span>
                <div className="border-input-border bg-surface flex h-28 items-center justify-center rounded-[12px] border border-dashed">
                  {logoPreview ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={logoPreview} alt="Logo preview" className="max-h-24 object-contain" />
                  ) : (
                    <span className="text-text-tertiary text-xs">No logo uploaded</span>
                  )}
                </div>
                <label className="text-small text-primary inline-flex cursor-pointer items-center gap-2 font-medium">
              ...
              <div className="flex flex-col gap-2">
                <span className="text-small text-text-primary font-semibold">Cover image</span>
                <div className="border-input-border bg-surface flex h-28 items-center justify-center overflow-hidden rounded-[12px] border border-dashed">
                  {coverPreview ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
                      src={coverPreview}
                    ...
                    <span className="text-text-tertiary text-xs">No cover uploaded</span>
                  )}
                </div>
                <label className="text-small text-primary inline-flex cursor-pointer items-center gap-2 font-medium">
                  <Icon name="Upload" className="size-4" aria-hidden />
          ...
          <AdminSection title="Rights & notes" icon="ShieldCheck">
            <div className="flex flex-col gap-4">
              <Field
                label="Image rights status"
                htmlFor="rightsStatus"
                error={errors.rightsStatus?.message}
              >
        ...
        <aside className="lg:block">
          <div className="sticky top-20 flex flex-col gap-4">
            ...
            <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
              <p className="text-small text-text-primary font-semibold">Readiness checklist</p>
              <ul className="text-small mt-2 flex flex-col gap-1.5">
                {readinessChecklist.map((item) => (
                  <li key={item.label} className="flex items-center gap-2">
                    <Icon
                      name={item.done ? "CheckCircle2" : "Circle"}
                      className={item.done ? "text-success size-4" : "text-text-tertiary size-4"}
                      aria-hidden
                    />
