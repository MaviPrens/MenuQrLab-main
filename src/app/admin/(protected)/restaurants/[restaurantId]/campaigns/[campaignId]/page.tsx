...
export default function CampaignEditorPage() {
  ...
  const canEdit =
    !!user &&
    (user.permissions.length === 0 || user.permissions.includes(PERMISSIONS.CAMPAIGN_EDIT));
    ...
    if (!isNew) {
      ...
      if (campaign) {
        reset({
          ...
          slug: campaign.slug,
          descriptionEn: campaign.localizedDescription
            ? resolveText(campaign.localizedDescription)
            : "",
  ...
  const onSubmit = handleSubmit((input) => {
    ...
    if (isNew) {
      ...
      toast({
        ...
        description: "Saved as a draft — nothing was published.",
        intent: "success",
      });
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6">
      ...
      <div className="border-info/30 bg-info/5 text-small text-info flex items-start gap-2 rounded-[12px] border p-3">
        ...
        <span>
          Saving creates or updates a draft only. Publishing is a separate, explicit step. No paid
          entry, gambling or automatic winner selection is supported.
        </span>
      </div>
        ...
        <div className="flex min-w-0 flex-col gap-6">
          <AdminSection title="Identity" icon="Info">
            <div className="flex flex-col gap-4">
              ...
              <Field
                ...
                hint="Who runs this campaign."
              >
                <Input
                  id="organizer"
                  placeholder={restaurant.displayName}
                  {...register("organizer")}
                />
              </Field>
            </div>
          ...
          <AdminSection title="Schedule" icon="CalendarClock">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <Field
                ...
                htmlFor="claimDeadline"
                error={errors.claimDeadline?.message}
              >
          ...
          <AdminSection title="Reward" icon="Gift">
            <div className="flex flex-col gap-4">
              <Field
                label="Reward title"
                htmlFor="rewardTitleEn"
                error={errors.rewardTitleEn?.message}
                required
              >
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <Textarea
                  id="eligibility"
                  rows={2}
                  placeholder="One reward per table per visit. Dine-in only."
              ...
              >
                <Textarea
                  id="attemptRules"
                  rows={2}
                  placeholder="A single check-in per visit. No paid attempts."
                  {...register("attemptRules")}
                />
              </Field>
            </div>
          </AdminSection>
            ...
            <AdminSection title="Danger zone" icon="ShieldAlert">
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <p className="text-small text-text-secondary">
                    Removing a campaign cannot be undone in the demo.
                  </p>
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-20 flex flex-col gap-4">
            <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
              <p className="text-small text-text-primary font-semibold">Public preview</p>
              <div className="border-border bg-surface-warm mt-3 overflow-hidden rounded-[16px] border">
                <div className="bg-navy px-4 py-3">
                  <p className="font-display text-button font-bold text-white">
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-small text-text-primary truncate font-semibold">
                        {rewardTitleEn || "Reward title"}
                      </p>
                      <p className="text-text-secondary text-xs">
                        {REWARD_TYPE_LABELS[rewardType ?? "free-item"]}
                      </p>
                    </div>
                  </div>
            ...
            <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
              <p className="text-small text-text-primary font-semibold">Status</p>
              <div className="mt-2 flex flex-col gap-2">
                <StatusBadge group="campaign" value={status ?? "draft"} />
                {existing ? (
                  <p className="text-text-tertiary text-xs">
                    Publishing: {titleCase(existing.publishingStatus)}
                  </p>
                ) : (
                  <p className="text-text-tertiary text-xs">Publishing: Draft</p>
                )}
              </div>
            </div>
          </div>
