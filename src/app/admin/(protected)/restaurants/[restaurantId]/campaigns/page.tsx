...
export default function RestaurantCampaignsPage() {
  ...
  const handleRemove = () => {
    ...
    recordAudit(
      "campaign.remove",
      removeTarget.id,
      `Removed campaign ${resolveText(removeTarget.localizedTitle)}`,
    );
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="min-w-0">
            ...
            <p className="text-text-secondary text-xs">/{row.original.slug}</p>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="min-w-0">
            <p className="text-small text-text-primary truncate">
              {resolveText(row.original.reward.title)}
            </p>
            <p className="text-text-secondary text-xs">
              {REWARD_TYPE_LABELS[row.original.reward.type]}
            </p>
          </div>
        ),
      },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="border-info/30 bg-info/5 text-small text-info flex items-start gap-2 rounded-[12px] border p-3">
        ...
        <span>
          Campaigns are simple thank-you mechanics — no paid entry, gambling or automatic winner
          selection. Participant counts shown elsewhere are illustrative demo data. Publishing is a
      ...
      {campaigns.length === 0 ? (
        <EmptyState
          ...
          description="Create the first campaign to offer a simple reward through QR entry points."
          icon="Megaphone"
          action={
            canEdit
              ? {
                  label: "Add campaign",
                  onClick: () => router.push(routes.admin.restaurantCampaign(id, "new")),
                }
              : undefined
          }
        />
