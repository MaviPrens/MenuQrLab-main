...
export default function RestaurantQRCodesPage() {
  ...
  const canManage =
    !!user && (user.permissions.length === 0 || user.permissions.includes(PERMISSIONS.QR_MANAGE));
  ...
  const metrics = useMemo(
    () => ({
      ...
      attention: records.filter(
        (r) => r.artworkStatus === "not-started" || r.artworkStatus === "in-progress",
      ).length,
    }),
    [records],
  );
  ...
  const recordAudit = (action: string, resourceId: string, description: string) => {
    demoStore.recordActivity({
      ...
      actorRole: user?.role ?? "qr-operations-manager",
      action,
  ...
  const handleSave = (input: QRCodeInput) => {
    ...
    if (editor.mode === "create") {
      ...
      toast({
        title: "QR record created",
        description: "Saved as a draft entry. Nothing was published.",
        intent: "success",
      });
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-3">
            <span className="bg-navy flex size-9 shrink-0 items-center justify-center rounded-[10px] text-white">
              <Icon name="QrCode" className="size-5" aria-hidden />
            </span>
            <div className="min-w-0">
              <p className="text-text-primary font-semibold">{row.original.displayIdentifier}</p>
              <p className="text-text-secondary text-xs">{row.original.placement}</p>
            </div>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span
            className="text-small text-text-secondary block max-w-[220px] truncate"
  ...
  const filters: FilterConfig[] = [
    ...
    },
    {
      ...
      options: [
        { label: "All", value: "all" },
        ...ARTWORK_STATUSES.map((s) => ({ label: ARTWORK_LABELS[s], value: s })),
      ],
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="border-info/30 bg-info/5 text-small text-info flex items-start gap-2 rounded-[12px] border p-3">
        ...
        <span>
          QR records map physical codes to public destinations. Scans of your own restaurant pages
          are attributed automatically in Analytics. Signing secrets and redirect keys are never
          shown here.
        </span>
      </div>
      ...
      {records.length === 0 ? (
        <EmptyState
          ...
          action={
            canManage
              ? { label: "Create QR", onClick: () => setEditor({ mode: "create", record: null }) }
              : undefined
          }
        />
      ) : (
...
function QRCodeEditorDialog({
  ...
  return (
    <div
      ...
      className="bg-navy-deep/40 fixed inset-0 z-50 flex items-center justify-center p-4"
      onKeyDown={(e) => {
        if (e.key === "Escape") onCancel();
      }}
    >
      <div className="border-border bg-canvas shadow-lift w-full max-w-lg rounded-[20px] border p-6">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit((input) => onSave(input))}>
          <Field
            ...
            htmlFor="qr-displayIdentifier"
            error={errors.displayIdentifier?.message}
            required
          >
          ...
          >
            <Input
              id="qr-destination"
              placeholder="/restaurants/pizza-house"
