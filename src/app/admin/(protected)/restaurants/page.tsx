...
export default function RestaurantsListPage() {
        ...
        cell: ({ row }) => (
          <div className="min-w-0">
            <Link
              ...
              className="text-text-primary hover:text-primary font-semibold"
            >
              {row.original.displayName}
            </Link>
            <p className="text-text-secondary text-xs">{titleCase(row.original.visualDirection)}</p>
          </div>
        ),
      },
          ...
          row.original.assignedTeams.length > 0 ? (
            <div className="flex flex-wrap gap-1">
              {row.original.assignedTeams.map((t) => (
                <span
                  key={t}
                  className="bg-surface-muted text-text-secondary rounded-full px-2 py-0.5 text-xs font-medium"
                >
                  {t}
                </span>
              ))}
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1">
            ...
            {row.original.publishingStatus !== "archived" ? (
              ...
              >
                <Link
                  href={routes.restaurant.home(row.original.slug)}
                  target="_blank"
                  rel="noreferrer"
                >
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader
        title="Restaurants"
        description="Every restaurant MenuQrLab manages. Open one to work on its setup, branding and content."
      ...
      <section aria-label="Summary metrics" className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      ...
      >
        <label className="text-small text-text-secondary mb-0.5 flex min-w-[160px] cursor-pointer items-center gap-2">
      ...
      {selectedIds.length > 0 ? (
        <div className="border-primary/30 bg-primary/5 flex flex-wrap items-center justify-between gap-3 rounded-[12px] border p-3">
          <p className="text-small text-text-primary font-medium">{selectedIds.length} selected</p>
          <div className="flex flex-wrap items-center gap-2">
            <Button
              variant="secondary"
              size="sm"
              onClick={() => applyBulk({ publishingStatus: "published" }, "published")}
            >
            ...
            <Button
              variant="secondary"
              size="sm"
              onClick={() => applyBulk({ publishingStatus: "draft" }, "unpublished")}
            >
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => applyBulk({ operationalStatus: "disabled" }, "disabled")}
            >
              <Icon name="Ban" className="size-4" aria-hidden />
              Disable
            </Button>
            <Button
              variant="outline"
              size="sm"
              onClick={() => applyBulk({ publishingStatus: "archived" }, "archived")}
            >
              <Icon name="Archive" className="size-4" aria-hidden />
              Archive
            </Button>
