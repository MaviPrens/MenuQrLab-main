...
export default function PlatformSettingsPage() {
  ...
  const persistSocialImage = (value: string | null) => {
    ...
    demoStore.recordActivity({
      ...
      resourceId: "settings",
      description: value
        ? "Updated default social sharing image"
        : "Removed default social sharing image",
    });
  };
  ...
  const onSocialUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (url) {
      ...
      toast({
        ...
        description: "Applied to public share previews.",
        intent: "success",
      });
    } else {
      toast({
  ...
  return (
    <form onSubmit={onSave} className="flex flex-col gap-6">
      ...
      <div className="border-info/30 bg-info/5 flex items-start gap-3 rounded-[16px] border p-4">
        <Icon name="Info" className="text-info mt-0.5 size-5 shrink-0" aria-hidden />
        ...
        <div className="flex min-w-0 flex-col gap-6">
          ...
          >
            <div className="flex flex-col gap-4">
              <Field
                ...
                hint="Use %s as the page-title placeholder, e.g. “%s · MenuQrLab”."
                required
              >
          ...
          <AdminSection title="Social image" icon="Image">
            <div className="flex flex-col gap-3">
              ...
              <div className="border-border bg-surface flex flex-col items-start gap-3 rounded-[12px] border border-dashed p-5 text-center sm:flex-row sm:items-center sm:text-left">
                <span className="bg-surface-warm text-primary flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-[12px]">
                  {socialImage ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  {socialImage ? (
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => persistSocialImage(null)}
                    >
                      Remove
                    </Button>
                  ) : null}
          ...
          <AdminSection title="Public experience" icon="Power">
            <label className="border-border bg-surface flex cursor-pointer items-start gap-3 rounded-[12px] border p-4">
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-20 flex flex-col gap-4">
            <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
              <p className="text-small text-text-primary font-semibold">Search preview</p>
              <div className="border-border bg-surface mt-3 rounded-[12px] border p-3">
                <p className="text-small text-info truncate font-medium">{previewTitle}</p>
                <p className="text-success truncate text-xs">yourplatform.example</p>
              ...
              <p className="text-text-tertiary mt-3 text-xs">
                Illustrative preview using “{siteName || "MenuQrLab"}”.
              </p>
            </div>
            {settings ? (
              <div className="border-border bg-canvas shadow-card rounded-[16px] border p-4">
                <p className="text-small text-text-primary font-semibold">Applied settings</p>
