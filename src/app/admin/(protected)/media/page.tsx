...
function MediaThumb({ row }: { row: MediaRow }) {
  ...
  return (
    <span className="border-border bg-surface relative block size-12 shrink-0 overflow-hidden rounded-[8px] border">
      <Image
        src={src}
  ...
  const columns = useMemo<ColumnDef<MediaRow, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Type",
        cell: ({ row }) => (
          <span className="text-text-secondary">{titleCase(row.original.type)}</span>
        ),
      },
      {
        ...
        header: "Rights",
        cell: ({ row }) => (
          <span className="border-border bg-surface text-text-secondary inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-semibold">
