  ...
  const columns = useMemo<ColumnDef<CampaignRow, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Reward",
        cell: ({ row }) => (
          <span className="text-text-secondary">{row.original.reward || "—"}</span>
        ),
      },
      {
        id: "actions",
        ...
        cell: ({ row }) => (
          <Button
            ...
            variant="ghost"
            size="sm"
            aria-label={`Edit campaign ${row.original.title}`}
          >
            <Link
              href={routes.admin.restaurantCampaign(row.original.restaurantId, row.original.id)}
            >
