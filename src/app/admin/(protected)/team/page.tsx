...
export default function TeamPage() {
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="min-w-0">
            <p className="text-text-primary font-semibold">{row.original.displayName}</p>
            <p className="text-text-secondary truncate text-xs">{row.original.email}</p>
          </div>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <PermissionGate
            ...
            fallback={<span className="text-text-tertiary text-xs">View only</span>}
          >
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader
        title="Users & Team"
        description="Internal MenuQrLab administrator accounts. Management actions require the Admin Manage permission."
      ...
      {!canManage ? (
        <div className="border-warning/30 bg-warning/5 flex items-start gap-3 rounded-[16px] border p-4">
          <Icon name="Lock" className="text-warning mt-0.5 size-5 shrink-0" aria-hidden />
