...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {attention.length > 0 ? (
        <div className="border-warning/30 bg-warning/5 flex items-start gap-3 rounded-[16px] border p-4">
          <Icon name="AlertTriangle" className="text-warning mt-0.5 size-5 shrink-0" aria-hidden />
        ...
        <div className="border-success/30 bg-success/5 flex items-start gap-3 rounded-[16px] border p-4">
          <Icon name="CheckCircle2" className="text-success mt-0.5 size-5 shrink-0" aria-hidden />
          <p className="text-small text-text-secondary">
              ...
              <ul className="divide-border flex flex-col divide-y">
                {attention.map((task, i) => (
                  <li key={`${task.href}-${i}`}>
                    <Link
                      href={task.href}
                      className="hover:text-primary focus-visible:outline-primary flex min-h-12 items-center gap-3 py-2 focus-visible:outline-2 focus-visible:outline-offset-2"
                    >
                      <span className="bg-surface-warm text-primary flex size-8 shrink-0 items-center justify-center rounded-[10px]">
                        <Icon name={task.icon} className="size-4" aria-hidden />
                      </span>
                      <span className="text-small text-text-primary flex-1">{task.label}</span>
                      <span className="bg-surface-muted text-text-secondary rounded-full px-2 py-0.5 text-xs font-semibold">
                        {task.badge}
                      </span>
                      <Icon name="ChevronRight" className="text-text-tertiary size-4" aria-hidden />
                    </Link>
                  </li>
                ))}
              </ul>
          ...
          <AdminSection title="Publishing" icon="Globe">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div
                  key={stage.key}
                  className="border-border bg-surface flex flex-col gap-2 rounded-[12px] border p-3"
                >
              ...
              {(
                ...
                return (
                  <li key={status} className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-small text-text-primary font-semibold">{count}</span>
                  </li>
                );
              })}
            </ul>
                  ...
                  {enquiriesR.data.slice(0, 4).map((e) => (
                    <li key={e.id} className="flex items-center justify-between gap-2 py-2.5">
                      <div className="min-w-0">
                        ...
                        <p className="text-text-secondary truncate text-xs">{e.contactPerson}</p>
                      </div>
