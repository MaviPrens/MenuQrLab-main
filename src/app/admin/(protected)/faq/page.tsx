...
export default function FaqPage() {
  ...
  const canEdit = Boolean(
    user &&
    (user.permissions.length > 0 ? user.permissions.includes(PERMISSIONS.WEBSITE_EDIT) : true),
  );
    ...
    () => [
      {
        ...
        header: "Category",
        cell: ({ row }) => (
          <span className="bg-surface-muted text-text-secondary rounded-full px-2 py-0.5 text-xs font-medium">
      ...
      },
      {
        ...
        header: "Question",
        cell: ({ row }) => (
          <span className="text-text-primary font-medium">{row.original.question}</span>
        ),
      },
      {
        accessorKey: "status",
      ...
      },
      {
        ...
        cell: ({ row }) =>
          canEdit ? (
            ...
            <span className="text-text-tertiary text-xs">View only</span>
          ),
      },
    ],
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ConfirmationDialog
        ...
        description={deleting ? `“${deleting.question}” will be permanently removed.` : undefined}
