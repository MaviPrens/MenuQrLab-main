...
const restaurantRepo: RestaurantRepository = {
  async list(query: RestaurantListQuery = {}) {
    ...
    if (query.operationalStatus)
      items = items.filter((r) => r.operationalStatus === query.operationalStatus);
    if (query.publishingStatus)
      items = items.filter((r) => r.publishingStatus === query.publishingStatus);
    if (query.setupStatus) items = items.filter((r) => r.setupStatus === query.setupStatus);
    if (query.visualDirection)
      items = items.filter((r) => r.visualDirection === query.visualDirection);
...
const menuRepo: MenuRepository = {
  ...
  async productBySlug(restaurantId, slug) {
    const [row] = await getDb()
      ...
      .from(schema.menuProducts)
      .where(
        and(eq(schema.menuProducts.restaurantId, restaurantId), eq(schema.menuProducts.slug, slug)),
      )
      .limit(1);
    return row?.data ?? null;
  },
...
const contentRepo: ContentRepository = {
  ...
  async websiteContent() {
    const rows = await getDb()
      .select({ data: schema.websiteContent.data })
...
const activityRepo: ActivityRepository = {
  ...
  async record(input) {
    ...
    await getDb().insert(schema.activity).values({
      id: created.id,
...
const authRepo: AuthRepository = {
  async findByEmail(email) {
    ...
    return (
      rows.map((r) => r.data).find((u) => u.email.toLowerCase() === email.toLowerCase()) ?? null
    );
  },
