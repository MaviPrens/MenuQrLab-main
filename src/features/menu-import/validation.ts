import { importResultSchema, type ImportResult, type ImportWarning } from "@/domain/menu-import";
...
export function validateImportResult(result: ImportResult): ValidationOutcome {
  ...
  const selectedProducts = selectedCategories.reduce(
    (n, c) => n + c.items.filter((i) => i.selectedForImport && i.reviewState !== "rejected").length,
    0,
  );
