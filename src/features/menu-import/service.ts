...
export const menuImportService = {
  ...
  ingestResult(
    ...
    this.appendEvent(
      importId,
      event("RUNNING_OCR", "RUNNING_OCR", 40, "Read pages with on-device OCR"),
    );
    this.appendEvent(
      importId,
      event("STRUCTURING_DATA", "STRUCTURING_DATA", 88, "Structured categories and products"),
    );
    return this.patch(importId, {
      processingStatus: "REVIEW_REQUIRED",
      reviewStatus: "IN_REVIEW",
  ...
  commit(
    importId: string,
    committedBy: { id: string | null; role: Role },
  ): { ok: boolean; reason?: string; created?: { categories: number; products: number } } {
    const rec = demoStore.menuImports.byId(importId);
    ...
    this.patch(importId, {
      ...
      result: {
        ...rec.result,
        statistics: {
          ...rec.result.statistics,
          manualCorrections: rec.result.statistics.manualCorrections,
        },
      },
    });
    ...
    return {
      ok: true,
      created: { categories: newCategories.length, products: newProducts.length },
    };
  },
