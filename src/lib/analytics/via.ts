/**
 * Tag a QR destination with a `?via=qr` marker so the analytics pipeline can
 * attribute the arrival channel. Only links to our own restaurant public pages
 * are tagged (external ordering sites can't be tracked and are left as-is),
 * and an existing `via` param is preserved. Pure + safe on empty input.
 */
export function addViaMarker(destination: string | null | undefined, channel: "qr"): string {
