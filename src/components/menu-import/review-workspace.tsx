...
export function ReviewWorkspace({ record, restaurantId, importId, user }: ReviewWorkspaceProps) {
  ...
  const onApprove = () => {
    ...
    toast({
      ...
      description: "Ready to commit to the draft menu.",
      intent: "success",
    });
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      {unresolvedBlocking.length > 0 ? (
        <div
          role="alert"
          className="border-danger/30 bg-danger/5 text-small text-danger flex items-start gap-2 rounded-[12px] border p-3"
        >
        ...
        info={
          <span className="flex items-center gap-2">
            <Icon
              ...
              className={validation.ok ? "text-success size-4" : "text-warning size-4"}
              aria-hidden
            />
            {validation.selectedProducts} product(s) selected · {unresolvedBlocking.length}{" "}
            unresolved blocking warning(s)
          </span>
        }
      >
        {!isApproved ? (
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        ...
        <AdminMetricCard
          ...
          value={s.highConfidenceFields}
          icon="ShieldCheck"
          intent="success"
        />
        <AdminMetricCard
          label="Medium confidence fields"
          value={s.mediumConfidenceFields}
          icon="ShieldAlert"
          intent="warning"
        />
        <AdminMetricCard
          label="Low confidence fields"
          value={s.lowConfidenceFields}
          icon="ShieldX"
        />
        <AdminMetricCard
...
function CategoriesPanel({
  ...
  if (result.categories.length === 0) {
    return (
      <p className="border-border bg-surface text-small text-text-secondary rounded-[12px] border border-dashed px-4 py-8 text-center">
          ...
          <PermissionGate
            ...
            fallback={
              <p className="text-text-secondary text-xs">
                You can review but not edit these candidates.
              </p>
            }
          >
            <div className="border-border bg-surface mb-4 flex flex-col gap-3 rounded-[12px] border p-3 sm:flex-row sm:items-end">
              ...
              <label className="text-small text-text-primary flex min-h-11 items-center gap-2">
                <Checkbox
                  checked={cat.selectedForImport}
                  onChange={(e) =>
                    onUpdateCategory(cat.candidateId, { selectedForImport: e.target.checked })
                  }
                />
                Include category
              </label>
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-text-primary font-medium">{item.name.value}</p>
            ...
            <Input
              ...
              onBlur={(e) => {
                if (e.target.value !== item.name.value) {
                  onUpdate({
                    name: { ...item.name, value: e.target.value, manuallyCorrected: true },
                  });
                }
              }}
            />
                ...
                if (value !== (item.description?.value ?? "")) {
                  onUpdate({
                    description: value
                      ? {
                          value,
                          confidence: item.description?.confidence ?? 1,
...
function PricesPanel({
  ...
  return (
    <div className="border-border bg-canvas shadow-card overflow-x-auto rounded-[16px] border">
      <table className="text-small w-full min-w-[760px] border-collapse text-left">
        <caption className="sr-only">Extracted prices</caption>
        <thead>
          <tr className="border-border text-text-tertiary border-b text-xs tracking-wide uppercase">
        ...
        <tbody>
          {rows.map(({ categoryId, item }) => {
            ...
            return (
              <tr key={item.candidateId} className="border-border border-b last:border-0">
                <th scope="row" className="text-text-primary px-4 py-2 font-medium">
                  {item.name.value}
                </th>
                <td className="text-text-secondary px-4 py-2 font-mono text-xs">
                  {price?.originalText ?? "—"}
                </td>
                <td className="text-text-primary px-4 py-2">
                  {price ? formatPrice(price.amount, currency) : "—"}
                </td>
                <td className="text-text-secondary px-4 py-2">{currency}</td>
                ...
                <td className="px-4 py-2">
                  <PermissionGate
                    user={user}
                    permission={PERMISSIONS.MENU_IMPORT_EDIT}
                    fallback={<span className="text-text-tertiary text-xs">—</span>}
                  >
                    <Input
...
function ImagesPanel({
  ...
  const withImages = result.categories.flatMap((cat) =>
    cat.items.filter((item) => item.image).map((item) => ({ categoryId: cat.candidateId, item })),
  );
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {withImages.map(({ categoryId, item }) => {
        ...
        return (
          <div
            key={item.candidateId}
            className="border-border bg-canvas shadow-card flex flex-col gap-3 rounded-[16px] border p-3"
          >
            <div className="bg-surface relative h-40 w-full overflow-hidden rounded-[12px]">
              <Image
            ...
            <p className="text-text-primary truncate font-medium">{item.name.value}</p>
            <div className="flex flex-wrap items-center gap-2">
              <ConfidenceChip value={image.confidence} />
              {image.requiresReview ? (
                <span className="bg-warning/10 text-warning inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold">
            ...
            <PermissionGate user={user} permission={PERMISSIONS.MENU_IMPORT_EDIT}>
              <div className="flex items-center gap-2">
                ...
                >
                  <Icon name="Trash2" className="text-danger size-4" aria-hidden />
                  Remove image
                </Button>
              </div>
            </PermissionGate>
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <code className="bg-surface-container text-text-secondary rounded px-1.5 py-0.5 text-xs">
            {warning.code}
          </code>
        </div>
        {warning.resolved ? (
...
function JsonPanel({ result, onDownload }: { result: ImportResult; onDownload: () => void }) {
  return (
    <div className="flex flex-col gap-3">
      ...
      <pre className="border-border bg-navy-deep text-surface max-h-[28rem] overflow-auto rounded-[12px] border p-4 text-xs leading-relaxed">
    ...
    >
      <dl className="grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
        {rows.map((row) => (
          <div
            key={row.label}
            className="border-border text-small flex items-center justify-between gap-3 border-b py-1.5 last:border-0"
          >
            <dt className="text-text-secondary">{row.label}</dt>
            <dd className="text-text-primary font-semibold">{row.value}</dd>
          </div>
        ))}
      </dl>
      <p className="border-info/30 bg-info/5 text-info mt-4 flex items-start gap-2 rounded-[12px] border p-3 text-xs">
