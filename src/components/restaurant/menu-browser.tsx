        ...
        {filtered.length === 0 ? (
          ...
          grouped.map((group) => (
            <section key={group.category.id}>
              <h2 className="font-heading text-h3 text-text-primary mb-3 flex items-center gap-2.5 font-bold">
                {group.category.image &&
                (group.category.image.startsWith("/") ||
                  group.category.image.startsWith("http")) ? (
                  <span className="bg-surface-container relative size-9 shrink-0 overflow-hidden rounded-[10px]">
                    <Image
