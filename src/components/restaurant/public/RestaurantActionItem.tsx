  ...
  const inner = (
    ...
    >
      <ActionIcon
        ...
        imgClassName="size-9"
      />
      <span className={cn("text-[11px] leading-tight font-semibold", active && "text-primary")}>
        {label}
      </span>
    </span>
  );
  ...
  if (mode === "disabled" || !href) {
    return (
      <li className="flex-1">
        <span
          aria-disabled="true"
          className="text-text-tertiary flex min-h-[60px] flex-col items-center justify-center gap-1 px-1 py-1.5 text-center opacity-60"
        >
          <ActionIcon
            icon={iconOverride}
            fallbackSrc={iconSrc}
