...
export function RestaurantFloatingContactMenu({
  ...
  return (
    <div
      ref={containerRef}
      className="fixed z-50 ltr:right-4 md:ltr:right-6 rtl:left-4 md:rtl:left-6"
      style={{
        bottom:
    ...
    >
      ...
      <ul
        ...
        className={cn(
          "absolute bottom-full mb-3 flex flex-col items-end gap-2.5 transition-all duration-200 ltr:right-0 rtl:left-0",
        ...
        {items.map((item, index) => (
          ...
          >
            <a
              ...
              className="focus-visible:outline-primary flex items-center gap-2.5 focus-visible:outline-2 focus-visible:outline-offset-2"
            >
              <span className="border-border bg-canvas text-small text-text-primary shadow-card rounded-full border px-3 py-1.5 font-semibold whitespace-nowrap">
                {item.label ?? (item.labelKey ? t(item.labelKey) : "")}
                {item.external ? <span className="sr-only"> — {t("rb.opensExternal")}</span> : null}
              </span>
              <span className="bg-primary shadow-lift flex size-11 items-center justify-center overflow-hidden rounded-full text-white">
                <ActionIcon
                  icon={item.icon}
                  size={20}
                  className="size-5"
                  imgClassName="size-6 rounded-sm"
                />
              </span>
            </a>
          </li>
      ...
      <button
        ...
        className="bg-primary shadow-lift hover:bg-primary-dark focus-visible:outline-primary flex size-14 items-center justify-center rounded-full text-white transition-transform duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 active:scale-95 ltr:ml-auto rtl:mr-auto"
      >
        <Icon
          name={open ? "X" : "Plus"}
