...
export function SocialActions({
  ...
  const social = actions.filter(
    (action) =>
      action.enabled &&
      (action.type === "whatsapp" || action.type === "email" || action.type === "instagram"),
  );
  ...
  return (
    <ul className={cn("grid grid-cols-2 gap-3 sm:grid-cols-3", className)}>
      {social.map((action) => {
        ...
        const inner = (
          <>
            <Icon name={icon} className="text-primary size-5" aria-hidden />
      ...
      {hasSaveContact ? (
        <li>
          <a
            ...
            className="border-border bg-canvas text-text-primary hover:bg-surface focus-visible:outline-primary flex min-h-[48px] items-center justify-center gap-2 rounded-[12px] border px-3 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2"
          >
            <Icon name="UserPlus" className="text-primary size-5" aria-hidden />
