...
export function AdminSidebar({ user }: AdminSidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "bg-navy sticky top-0 hidden h-dvh shrink-0 flex-col border-r border-white/10 text-white transition-[width] duration-200 lg:flex",
        collapsed ? "w-[76px]" : "w-64",
      )}
      aria-label="Admin navigation"
    >
      <div className="flex h-16 items-center gap-2.5 border-b border-white/10 px-4">
        <span className="bg-primary flex size-9 shrink-0 items-center justify-center rounded-[10px] text-white">
          <Icon name="QrCode" className="size-5" aria-hidden />
        </span>
        {!collapsed ? (
          <div className="min-w-0">
            <p className="font-heading text-button truncate font-bold">MenuQrLab</p>
                  ...
                  return (
                    <li key={item.href}>
                      <Link
                        ...
                        className={cn(
                          "group text-small focus-visible:outline-primary flex min-h-11 items-center gap-3 rounded-[10px] px-3 font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2",
                          active
      ...
      <div className="border-t border-white/10 p-3">
        <button
          ...
          className="text-small focus-visible:outline-primary flex min-h-11 w-full items-center justify-center gap-2 rounded-[10px] px-3 font-medium text-white/60 hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2"
