  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6 pb-4">
      ...
      <AdminSection
        title="Page metadata"
        icon="FileText"
        description={`${titleCase(page.type)} · ${page.locale.toUpperCase()}`}
      >
        ...
        {sections.length === 0 ? (
          ...
          <ul className="flex flex-col gap-5">
            {sections.map((section, index) => (
              <li
                key={section.id}
                className="border-border bg-surface flex flex-col gap-3 rounded-[12px] border p-4"
              >
                <div className="flex items-start gap-2">
                  <div className="flex-1">
                    <Field label={`Section ${index + 1} title`} htmlFor={`sec-title-${section.id}`}>
                      <Input
                ...
                <div className="border-border bg-canvas rounded-[10px] border p-3">
                  <p className="text-text-tertiary mb-1 text-xs font-semibold tracking-wide uppercase">
                    Preview (plain text)
                  </p>
                  {section.body.trim() === "" ? (
      ...
      {canEdit ? (
        <StickyActionBar info="Changes save to the database and appear on the public legal page once set to Published.">
          <Button type="button" variant="ghost" onClick={onBack}>
            Cancel
          </Button>
          <Button type="submit">
        ...
        <div className="border-border bg-surface text-small text-text-secondary rounded-[12px] border p-4">
