...
interface EnquiryDetailDrawerProps {
  ...
  form: UseFormReturn<
    z.input<typeof enquiryUpdateSchema>,
    unknown,
    z.output<typeof enquiryUpdateSchema>
  >;
  teamOptions: string[];
  onClose: () => void;
  onSubmit: () => void;
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="border-border flex flex-col gap-0.5 border-b py-2.5 last:border-0">
      <dt className="text-text-tertiary text-xs font-semibold tracking-wide uppercase">{label}</dt>
  ...
  return (
    <div className="fixed inset-0 z-[80] flex justify-end">
      ...
      <div
        ...
        tabIndex={-1}
        className="border-border bg-canvas shadow-lift relative flex h-full w-full max-w-md flex-col overflow-y-auto border-l focus-visible:outline-none"
      >
        ...
        <div className="flex-1 p-5">
          ...
          <form
            ...
            className="border-border bg-surface mt-6 flex flex-col gap-4 rounded-[16px] border p-4"
          >
            <p className="text-small text-text-primary font-semibold">Update lead</p>
