...
export function RestaurantWorkspaceTabs({ restaurantId }: RestaurantWorkspaceTabsProps) {
  ...
  const tabs: TabDef[] = [
    ...
    {
      label: "Actions",
      href: routes.admin.restaurantActions(restaurantId),
    ...
    {
      label: "Analytics",
      href: routes.admin.restaurantAnalytics(restaurantId),
      icon: "ChartColumn",
    },
    {
      label: "Page Builder",
      href: routes.admin.restaurantPageBuilder(restaurantId),
      icon: "LayoutTemplate",
    },
  ];
  ...
  return (
    <div className="-mx-1 overflow-x-auto">
      <nav
        className="border-border flex min-w-max gap-1 border-b px-1"
        ...
        {tabs.map((tab) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "text-small focus-visible:outline-primary flex min-h-11 items-center gap-2 border-b-2 px-3 font-medium whitespace-nowrap transition-colors focus-visible:outline-2 focus-visible:outline-offset-2",
                active
            ...
            >
              ...
              {tab.label}
              {tab.partTwo ? (
                <span className="bg-surface-muted text-text-tertiary rounded-full px-1.5 py-0.5 text-[10px] font-semibold">
                  P2
                </span>
              ) : null}
            </Link>
