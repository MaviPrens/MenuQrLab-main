      ...
      {items.map((item, i) => (
        <li key={item.id} className="flex gap-3">
          <div className="flex flex-col items-center">
            <span className="bg-surface-warm text-primary flex size-8 shrink-0 items-center justify-center rounded-full">
              <Icon name={iconForAction(item.action)} className="size-4" aria-hidden />
            </span>
            {i < items.length - 1 ? <span className="bg-border w-px flex-1" aria-hidden /> : null}
          </div>
