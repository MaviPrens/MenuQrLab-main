...
export function MenuPdfManager({ restaurantId }: { restaurantId: string }) {
  ...
  const onPick = async (file: File) => {
    ...
    if (file.size > MAX_BYTES) {
      toast({
        ...
        description: "The PDF must be 4 MB or smaller.",
        intent: "danger",
      });
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-3">
      {meta ? (
        <div className="border-border bg-canvas flex flex-wrap items-center justify-between gap-3 rounded-[12px] border p-3">
          <div className="flex min-w-0 items-center gap-3">
            <Icon name="FileText" className="text-primary size-5 shrink-0" aria-hidden />
            <div className="min-w-0">
              <p className="text-small text-text-primary truncate font-semibold">{meta.filename}</p>
              <p className="text-text-secondary text-xs">{formatBytes(meta.fileSize)}</p>
            </div>
          </div>
      ...
      <div>
        ...
        <p className="text-text-tertiary mt-1.5 text-xs">
          PDF up to 4 MB. Stored and served from your database.
        </p>
      </div>
    </div>
  );
