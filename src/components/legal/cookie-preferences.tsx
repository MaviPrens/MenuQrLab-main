...
export function CookiePreferences({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      <div className="border-border bg-surface flex items-center gap-3 border-b px-5 py-4 md:px-6">
        <span className="bg-surface-warm text-primary flex size-9 items-center justify-center rounded-full">
      ...
      <div className="space-y-4 p-5 md:p-6">
        <p
          className="border-info/20 bg-info/5 text-small text-text-secondary rounded-[12px] border p-3"
          role="note"
        >
          <Icon
            name="Info"
        ...
        <fieldset className="space-y-3" aria-describedby={`${groupId}-legend`}>
          ...
          {COOKIE_CATEGORIES.map((category) => {
            ...
            return (
              <div
                key={category}
                className="border-border bg-canvas flex items-start justify-between gap-4 rounded-[12px] border p-4"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <label
                      htmlFor={controlId}
                      className="text-small text-text-primary block font-semibold"
                    >
                      {meta.label}
                      {locked ? (
                        <span className="bg-surface text-text-secondary ml-2 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium">
                          <Icon name="Lock" className="size-3" aria-hidden /> Always active
                        </span>
                      ) : null}
                    </label>
                    <p id={`${controlId}-desc`} className="text-small text-text-secondary mt-1">
                      {meta.description}
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <span
                    ...
                    className={cn(
                      ...
                      "after:absolute after:top-[2px] after:left-[2px] after:size-5 after:rounded-full after:bg-white after:shadow-sm after:transition-all after:content-['']",
        ...
        <div className="border-border flex flex-col gap-2 border-t pt-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
          <Button
            type="button"
            variant="outline"
        ...
        <div className="border-border text-small text-text-secondary flex flex-col items-start justify-between gap-2 border-t pt-4 sm:flex-row sm:items-center">
          <p aria-live="polite">
