      ...
      <Card className="border-success/30">
        <div className="flex flex-col items-center gap-4 p-8 text-center">
          ...
          {submitState.reference ? (
            <p className="bg-surface text-small text-text-secondary rounded-[12px] px-4 py-2">
              Reference:{" "}
              <span className="text-text-primary font-semibold">{submitState.reference}</span>
            </p>
          ) : null}
    ...
    <form onSubmit={onSubmit} noValidate className="space-y-6">
      ...
      <Card>
        <div className="space-y-5 p-6">
          <div className="flex items-center gap-2">
            <span className="bg-surface-warm text-small text-primary flex size-7 items-center justify-center rounded-full font-bold">
              1
            </span>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            >
              <Select id="restaurantType" defaultValue="" {...register("restaurantType")}>
      ...
      <Card>
        <div className="space-y-5 p-6">
          ...
          {/* Conditional: qr product → product interest checkboxes */}
          {showProducts ? (
            <Controller
              control={control}
              ...
              render={({ field }) => {
                ...
                return (
                  <fieldset className="flex flex-col gap-2">
                    <legend className="text-small text-text-primary mb-1 font-semibold">
                      Product interest
                    </legend>
                      ...
                      {productOptions.map((option) => {
                        ...
                        return (
                          <label
                            key={option}
                            htmlFor={id}
                            className="border-border bg-canvas text-small text-text-primary flex items-center gap-2.5 rounded-[12px] border p-3"
                          >
                            <Checkbox
                              id={id}
                              checked={checked}
          ...
          {showMultiLocation ? (
            <p className="border-info/20 bg-info/5 text-small text-text-secondary rounded-[12px] border p-3">
              <Icon
                name="Info"
                className="text-info mr-1.5 inline size-4 align-text-bottom"
                aria-hidden
              />
        ...
        <div className="space-y-5 p-6">
          ...
          <fieldset className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              {CONTACT_METHODS.map((method) => (
                <label
                  key={method}
                  className="border-border bg-canvas text-small text-text-primary has-[:checked]:border-primary has-[:checked]:bg-surface-warm flex cursor-pointer items-center gap-2 rounded-[12px] border px-4 py-2.5"
                >
                  <input
                    type="radio"
                    value={method}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {errors.consent ? (
                <p id="consent-error" role="alert" className="text-danger mt-1 text-xs font-medium">
      ...
      {formError ? (
        <div
          role="alert"
          className={cn(
            "text-small flex items-start gap-3 rounded-[12px] border p-4",
